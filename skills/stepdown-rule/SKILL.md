---
name: stepdown-rule
description: Write, refactor, or review functions in the stepdown style — top-down reading order, one level of abstraction per function, helpers extracted so orchestration stays separate from detail. Use when asked to write a nontrivial multi-step function, untangle a long function, extract helpers, or make code easier to read with clean-code decomposition, even if "stepdown" is never mentioned.
