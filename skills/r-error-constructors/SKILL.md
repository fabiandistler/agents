...
environments: coding
description: Design custom condition/error constructors for R packages the tidyverse/rlang way. They turn a recurring `stop()` call into a testable, documentable, and inheritable error type.
...
Whenever an R package raises the same error at three or more call sites,
whenever tests assert on error message text with a regex, or when a user asks
about `rlang::abort()`, custom conditions, `conditionMessage()` methods, or
building an error-class hierarchy in R.
