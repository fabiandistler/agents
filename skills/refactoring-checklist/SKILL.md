...
description: Decide whether, when, and how to refactor a spotted code smell — is it worth doing now, is it safe to touch, and how to change it in small reversible steps. Use it whenever the question is not "how do I perform this refactoring" but "should I refactor this right now, and how do I do it safely". Covers prioritizing a backlog of smells, choosing between a normal refactor, the Mikado Method, Strangler Fig, or a rewrite, and running reversible steps under the Two-Hats rule. Includes a priority matrix (immediate / this week / next sprint / litter-pickup) with concrete code smells — among them an R-specific set (data.table chains, `<<-` misuse, growing objects in loops, `T`/`F` vs `TRUE`/`FALSE`, `1:length(x)` vs `seq_along()`, unclosed DBI connections) — a test-coverage and change-size risk gate, and quality gates to confirm the refactor helped.
